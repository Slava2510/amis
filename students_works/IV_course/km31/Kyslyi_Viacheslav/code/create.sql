...
create or replace view rest_meal_full_view
as select r.ID AS rest_id, m.ID AS meal_id, m."Name", m."Description", m."Price" from MEALS m 
  INNER JOIN MENUS on MENUS.ID = m.MENU_ID
  INNER JOIN RESTAURANTS r on r.ID = MENUS.REST_ID;

CREATE  OR  REPLACE VIEW restView AS 
